...
// Fetch data from API
fetch('http://localhost:8081/pidev/api/auth/stats')
  .then(response => {
  ...
  .then(data => {
    ...
    // Extract data values from the API response
    var adminCount = data.adminCount;
    var candidateCount = data.condidateCount    ;
    var recruiterCount = data.recuiterCount;
    console.log(recruiterCount);

    // Set the chart options with the updated data
    var options = {
      tooltip: {
        trigger: 'item'
      },
